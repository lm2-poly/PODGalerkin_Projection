...
from projection_matrices_2W_stokes import *
...
all_Ur = np.arange(1.0,13.0,0.2)
# all_Ur = np.array([1,2,3,3.5,4,4.5,5,6,7,8,9,10,11,12,13])
...
# Effetive Reynolds number
Re_eff = 40.

def sigmas_xy_MC_forall_Ur(x, y, Ur):
    M = nCond.Mass_number
    sigma_x_vect = np.zeros(N_modes+4)
    sigma_y_vect = np.zeros(N_modes+4)
    
    F_drag_stokes = 6*np.pi /Re_eff *4 / (np.pi * nCond.Mass_number)
    ...
    sigma_x_vect[N_modes+1] = - M/(1+M) * (nCond.Zeta) - F_drag_stokes

    sigma_y_vect[N_modes+2] = - M/(1+M)* (2*np.pi/Ur)**2
    sigma_y_vect[N_modes+3] = - M/(1+M) * nCond.Zeta - F_drag_stokes
...
def Lambda_matrix_MC_forall_Ur(mean_mode, modes, x, y, Ur):
    ...
    lambda_matrix[N_modes +1,:], lambda_matrix[N_modes +3,:] = sigmas_xy_MC_forall_Ur(x, y, Ur)    
...
for i_ur in tqdm(range(len(all_Ur))):
    ...
    Lambda_mat[N_modes +1,:], Lambda_mat[N_modes +3,:] = sigmas_xy_MC_forall_Ur(x_np, y_np, Ur_val)    
    ...
    C_D =  Fx_mean_CFD * nCond.include_added_damping_force *np.pi*nCond.Mass_number/2#*7.5 #-gamma_bar_x_value*np.pi*nCond.Mass_number/2 
    # ------------------------------------------------------------------------------------------------------------------
    ...
    def Fq_AM_ND(a):
        ...
        for i in range(N_modes):
            Fq_i = -np.matmul(a,np.matmul(Q_np[i]-D_mat[i],a))
            Fq_list.append(Fq_i)
        Fq_list.append(0.0)
    ...
    if extrapol_test :
        ...
        POD_energies_Ur = np.sum(np.square(sol_ext.y[:N_modes]), axis=1)/np.sum(np.square(sol_ext.y[:N_modes]))
        POD_energies.append(POD_energies_Ur)
...
# from POD.POD import get_relative_energies_from_time_coeffs

# energies_ext = get_relative_energies_from_time_coeffs(sol_ext.y[:N_modes])

from matplotlib import colors
cmap = plt.cm.jet
norm = colors.BoundaryNorm(np.arange(0, 14, 1), cmap.N)


plt.figure(figsize=(10,10))
for i_ur in range(len(all_Ur)):
    plt.plot(np.arange(1,N_modes+1), POD_energies[i_ur], '-o',color = cmap(i) , label = "$U_r = {}$".format(all_Ur[i_ur]))
plt.xlabel("Order of the mode")
plt.ylabel("% of total energy")
plt.title("Energy captured by each POD mode")
plt.legend()
plt.show()

# pd.DataFrame(energies_ext).to_csv("Unified_PODVM_Galerkin\\Misc\\energy_fsi\\relative_energies_2W_ROM_Ur{}.csv".format(all_Ur[-1]))
captured_energies_pd = pd.DataFrame(POD_energies)
captured_energies_pd.columns= ["mode_{}".format(i) for i in range(1, 8+1)]
captured_energies_pd.insert(0, column="values_Ur", value= all_Ur)
# captured_energies_pd.to_csv("Misc\\energy_fsi\\relative_captured_energies_2W_ROM_all_Ur.csv", index=None)



reference_amplitudes = pd.read_csv("Misc\\data_amplitudes_force_coeffs.csv")

plt.figure(figsize=(10,12))
plt.plot(all_Ur[:],eta_amplitudes_max[:], color='darkblue', marker = 's', label = "2W ROM, Stokes modes.")
plt.plot(FSI_sim_Ur_x, true_eta_max, 'k-', marker='o', markerfacecolor='none', markeredgecolor='k' )
plt.plot(reference_amplitudes['ur'],reference_amplitudes['eta'], label = '2W ROM, inviscid modes',color="#004e7c", marker = 'o')
plt.xlabel("$U_r$")
plt.ylabel("$\eta_{max}$")
plt.legend()
plt.show()
...
plt.figure(figsize=(10,12))
plt.plot(all_Ur[:],xi_amplitudes_max[:], color="#FF7F00", marker = 's',label = "2W ROM, Stokes modes")
plt.plot(FSI_sim_Ur_y, true_xi_max, 'k-', marker='o', markerfacecolor='none', markeredgecolor='k' )
plt.plot(reference_amplitudes['ur'],reference_amplitudes['xi'], label = '2W ROM, inviscid modes',color='darkred', marker = 'o')
plt.legend()
plt.xlabel("$U_r$")
plt.ylabel("$\\xi_{max}$")
plt.show()
...
amplitude_data = pd.DataFrame({
    "ur": all_Ur,
    "eta_max": eta_amplitudes_max,
    "xi_max": xi_amplitudes_max
    }
    )
amplitude_data.to_csv("stokesROM\\data\\amplitude_data_Re_eff_{:.2f}".format(Re_eff), index=None)