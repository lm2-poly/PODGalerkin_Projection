"""
This code aims at reproducing the experimental results obtained by A. Schmider in her PhD thesis,
see http://www.theses.fr/2023POIT2253.
The interest for this work is that the experimental setup allows to study VIV oscillation response of a cylinder
subject to a flow, with the possibility of continuously varying the paremeters of the oscillator. 

The thesis has measurements of the amplitude and frequency of the oscillation response, for multiple values of mass
number, damping and reduced velocity. As we are able to easily vary those parameters in our code, we want to compare our 
results with the measurements from the thesis.  
"""
...
all_Zeta = [-0.005, 0.02, 0.041, 0.061, 0.082, 0.103, 0.144,0.186, 0.2, 0.3, 0.5, 0.6, 0.7, 0.8, 0.9, 1., 1.2, 1.5, 2.0]
# all_Zeta = [-0.005, 0.02, 0.144,0.186]
# all_Ur = np.array([6.0])
...
eta_max_all_zeta = []
xi_max_all_zeta = []
frequencies_all_zeta = []

for i_zeta in tqdm(range(len(all_Zeta))):

    Zeta_val = all_Zeta[i_zeta]
    eta_amplitudes_max = []
    xi_amplitudes_max = []
    ...
    for i_ur in range(len(all_Ur)):
        ...
        Lambda_mat[N_modes +1,:], Lambda_mat[N_modes +3,:] = sigmas_xy_MC_forall_Ur_Zeta(x_np, y_np, Ur_val, Zeta_val)    
        ...
        C_D =  Fx_mean_CFD*np.pi*nCond.Mass_number/2 *nCond.include_added_damping_force #*7.5 #-gamma_bar_x_value*np.pi*nCond.Mass_number/2 
        # ------------------------------------------------------------------------------------------------------------------
    ...
    eta_max_all_zeta.append(eta_amplitudes_max)    
    xi_max_all_zeta.append(xi_amplitudes_max)
    frequencies_all_zeta.append(late_frequencies)
    ...
    # psd_all_ur = np.array(psd_all_ur[0])
...
# Plot cross-flow maximum vibration amplitudes xi for all zeta -----------------------------------------------------------------
...
for i_zeta in range(len(all_Zeta)):
    plt.scatter(all_Ur,xi_max_all_zeta[i_zeta], marker = 's', label = "$\\zeta = {}$".format(str(all_Zeta[i_zeta])), color = cmap_xi(i_zeta*10+60))
# plt.plot(FSI_sim_Ur_y, true_xi_max, 'k-', marker='o', markerfacecolor='none', markeredgecolor='k' )
plt.xlabel("$U_r$")
plt.ylabel("$\\xi_{max}$")
plt.legend()
...
plt.figure(figsize=(10,12))
for i_zeta in range(len(all_Zeta)):
    plt.scatter(all_Ur,eta_max_all_zeta[i_zeta],  marker = 's', label = "$\\zeta = {}$".format(str(all_Zeta[i_zeta])), color = cmap_eta((i_zeta*10+60)))
# plt.plot(FSI_sim_Ur_x, true_eta_max, 'k-', marker='o', markerfacecolor='none', markeredgecolor='k' )
plt.xlabel("$U_r$")
plt.ylabel("$\eta_{max}$")
plt.legend()
...
frequencies_all_zeta = np.array(frequencies_all_zeta)
frequencies_all_zeta_main = frequencies_all_zeta[:,:,0]

cmap_f = plt.cm.BuGn
plt.figure(figsize=(10,12))
for i_zeta in range(7):
    plt.scatter(all_Ur,frequencies_all_zeta[i_zeta,:,0]*all_Ur* np.sqrt(2), marker = 's', color=cmap_f(i_zeta*15+70), label = "$\\zeta = {}$".format(str(all_Zeta[i_zeta])))
