...
CD_0 = Fx_mean_CFD*np.pi*nCond.Mass_number/2 *nCond.include_added_damping_force
CD_coeff_min, CD_coeff_max = 1.0, 20.0 # explore values from x 1 to x 20  of the original value
CD_coeff_step = 0.5  # step of 10% to explore the range of values

all_CD = np.arange(CD_coeff_min, CD_coeff_max, CD_coeff_step ) * CD_0
...
for i_CD in tqdm(range(len(all_CD))):
    # Drag force coefficient -------------------------------------------------------------------------------------------
    C_D =  all_CD[i_CD]
    print("Solving the ROM for C_D = {:.2f}".format(C_D))
