...
# ======================================================================================================================
# Projection matrices with added mass modes 

# ----------------------------------------------------------------------------------------------------------------------
# Functions to compute the matrices with the added mass modes
...
def phi_AM_v1(x, y):
    """
    For (x,y), returns the velocity added mass mode (two components, u and v) for the 1st AM mode (displacement in x )
    The mode will then be multiplied by the velocity of the cylinder xi
    """
    r, theta = cartesian_to_polar(x, y)

    phi_ux = 1/(4*r**2) * tf.cos(2*theta)
    phi_uy = 1/(4*r**2) * tf.sin(2*theta)
...
@tf.function
def phi_AM_v_mode_1_grad_y(x, y):
    phi_AM_v_mode_1 = phi_AM_v_mode_1(x, y)
    phi_AM_grad_y = tf.gradients(phi_AM_v_mode_1, y)[0]
    return phi_AM_grad_y
...
@tf.function
def phi_AM_v_mode_2_grad_y(x, y):
    phi_AM_v_mode_2 = phi_AM_v_mode_2(x, y)
    phi_AM_grad_y = tf.gradients(phi_AM_v_mode_2, y)[0]
    return phi_AM_grad_y

@tf.function
def Q_velocity_mean_AM_1(mean, x, y):
    """ 
    ...
    phi_AM_x, phi_AM_y = phi_AM_v1(x,y)
    phi_AM_x_dx, phi_AM_y_dx = phi_AM_v1_x_grad_x(x, y), phi_AM_v1_y_grad_x(x, y)
    phi_AM_x_dy, phi_AM_y_dy = phi_AM_v1_x_grad_y(x,y), phi_AM_v1_y_grad_y(x,y) 
    
    Q_u = tf.math.multiply(vec_mean_u, phi_AM_x_dx) + tf.math.multiply(vec_mean_v, phi_AM_x_dy) +\
        tf.math.multiply(phi_AM_x, vec_mean_u_dx) + tf.math.multiply(phi_AM_y, vec_mean_u_dy)
    Q_v = tf.math.multiply(vec_mean_u, phi_AM_y_dx) + tf.math.multiply(vec_mean_v, phi_AM_y_dy)+\
        tf.math.multiply(phi_AM_x, vec_mean_v_dx) + tf.math.multiply(phi_AM_y, vec_mean_v_dy) 

    return tf.concat([Q_u,Q_v],axis = 0)

@tf.function
def Q_velocity_mean_AM_2(mean, x, y):
    """ 
    Returns Q(mean, phi_AM) + Q(phi_AM, mean)
    """
    ...
    phi_AM_2_x, phi_AM_2_y = phi_AM_v2(x,y)
    phi_AM_2_x_dx, phi_AM_2_y_dx = phi_AM_v2_x_grad_x(x, y), phi_AM_v2_y_grad_x(x, y)
    phi_AM_2_x_dy, phi_AM_2_y_dy = phi_AM_v2_x_grad_y(x,y), phi_AM_v2_y_grad_y(x,y) 

    
    Q_u = tf.math.multiply(vec_mean_u, phi_AM_2_x_dx) + tf.math.multiply(vec_mean_v, phi_AM_2_x_dy) +\
        tf.math.multiply(phi_AM_2_x, vec_mean_u_dx) + tf.math.multiply(phi_AM_2_y, vec_mean_u_dy)
    Q_v = tf.math.multiply(vec_mean_u, phi_AM_2_y_dx) + tf.math.multiply(vec_mean_v, phi_AM_2_y_dy)+\
        tf.math.multiply(phi_AM_2_x, vec_mean_v_dx) + tf.math.multiply(phi_AM_2_y, vec_mean_v_dy) 

    return tf.concat([Q_u,Q_v],axis = 0)


def phi_i_nabla_phi_AM1(modes, i, x, y):
    """ 
    ...
    Q_u = tf.math.multiply(vec_u, phi_AM_x_dx) + tf.math.multiply(vec_v, phi_AM_x_dy)
    Q_v = tf.math.multiply(vec_u, phi_AM_y_dx) + tf.math.multiply(vec_v, phi_AM_y_dy)

    return tf.concat([Q_u,Q_v],axis = 0)

@tf.function
def phi_AM1_nabla_phi_i(modes, i, x, y):
    """ 
    Returns (phi_i . nabla )phi_AM
    """
    spatial_points = tf.transpose(tf.stack([x,y]))
    ...
    Q_u = tf.math.multiply(phi_AM_x, vec_u_dx ) + tf.math.multiply(phi_AM_y, vec_u_dy)
    Q_v = tf.math.multiply(phi_AM_x, vec_v_dx) + tf.math.multiply(phi_AM_y, vec_v_dy)
    
    return tf.concat([Q_u,Q_v],axis = 0)

def phi_i_nabla_phi_AM2(modes, i, x, y):
    """ 
    Returns (phi_i . nabla )phi_AM
    """
    spatial_points = tf.transpose(tf.stack([x,y]))
    ...
    phi_AM_x_dx, phi_AM_y_dx = phi_AM_v2_x_grad_x(x, y), phi_AM_v2_y_grad_x(x, y)
    phi_AM_x_dy, phi_AM_y_dy = phi_AM_v2_x_grad_y(x,y), phi_AM_v2_y_grad_y(x,y) 
...
@tf.function
def phi_AM2_nabla_phi_i(modes, i, x, y):
    """ 
    Returns (phi_i . nabla )phi_AM
    """
    spatial_points = tf.transpose(tf.stack([x,y]))
...
def phi_AM1_nabla_phi_AM1(x, y):
    ...
    Q_u = tf.math.multiply(phi_AM_x, phi_AM_x_dx) + tf.math.multiply(phi_AM_y, phi_AM_x_dy)
    Q_v = tf.math.multiply(phi_AM_x, phi_AM_y_dx) + tf.math.multiply(phi_AM_y, phi_AM_y_dy)

    return tf.concat([Q_u,Q_v],axis = 0)

def phi_AM1_nabla_phi_AM2(x, y):
    """ 
    Returns (phi_AM1 . nabla )phi_AM2
    """
    phi_AM1_x, phi_AM1_y = phi_AM_v1(x,y)
    
    phi_AM2_x_dx, phi_AM2_y_dx = phi_AM_v2_x_grad_x(x, y), phi_AM_v2_y_grad_x(x, y)
    phi_AM2_x_dy, phi_AM2_y_dy = phi_AM_v2_x_grad_y(x,y), phi_AM_v2_y_grad_y(x,y) 

    
    Q_u = tf.math.multiply(phi_AM1_x, phi_AM2_x_dx) + tf.math.multiply(phi_AM1_y, phi_AM2_x_dy)
    Q_v = tf.math.multiply(phi_AM1_x, phi_AM2_y_dx) + tf.math.multiply(phi_AM1_y, phi_AM2_y_dy)

    return tf.concat([Q_u,Q_v],axis = 0)

def phi_AM2_nabla_phi_AM1(x, y):
    """ 
    Returns (phi_AM2 . nabla )phi_AM1
    """
    phi_AM2_x, phi_AM2_y = phi_AM_v2(x,y)
    
    phi_AM1_x_dx, phi_AM1_y_dx = phi_AM_v1_x_grad_x(x, y), phi_AM_v1_y_grad_x(x, y)
    phi_AM1_x_dy, phi_AM1_y_dy = phi_AM_v1_x_grad_y(x,y), phi_AM_v1_y_grad_y(x,y) 

    
    Q_u = tf.math.multiply(phi_AM2_x, phi_AM1_x_dx) + tf.math.multiply(phi_AM2_y, phi_AM1_x_dy)
    Q_v = tf.math.multiply(phi_AM2_x, phi_AM1_y_dx) + tf.math.multiply(phi_AM2_y, phi_AM1_y_dy)

    return tf.concat([Q_u,Q_v],axis = 0)

def phi_AM2_nabla_phi_AM2(x, y):
    """ 
    Returns (phi_AM2 . nabla )phi_AM1
    """
    phi_AM2_x, phi_AM2_y = phi_AM_v2(x,y)
    ...
    Q_u = tf.math.multiply(phi_AM2_x, phi_AM2_x_dx) + tf.math.multiply(phi_AM2_y, phi_AM2_x_dy)
    Q_v = tf.math.multiply(phi_AM2_x, phi_AM2_y_dx) + tf.math.multiply(phi_AM2_y, phi_AM2_y_dy)

    return tf.concat([Q_u,Q_v],axis = 0)

def psi_p1(x, y):
    """
    For (x,y), returns the first pressure added mass mode
    The mode will then be multiplied by the acceleration of the cylinder d2 eta / d t2
    """
    r, theta = cartesian_to_polar(x, y)
    return tf.cos(theta) / (4*r)

def psi_p2(x, y):
    """
    For (x,y), returns the second pressure added mass mode
    The mode will then be multiplied by the squared velocity of the cylinder (d eta / d t)^2
    """
    r, theta = cartesian_to_polar(x, y)
    return - 1 /(32*r**4)

@tf.function
def psi_p1_grad_x(x,y):
    psi_1_AM = psi_p1(x,y)
    psi_p1_dx = tf.gradients(psi_1_AM,x)[0]
    return psi_p1_dx

@tf.function
def psi_p1_grad_y(x,y):
    psi_1_AM = psi_p1(x,y)
    psi_p1_dy = tf.gradients(psi_1_AM,y)[0]
    return psi_p1_dy

@tf.function
def psi_p2_grad_x(x,y):
    psi_2_AM = psi_p2(x,y)
    psi_p2_dx = tf.gradients(psi_2_AM,x)[0]
    return psi_p2_dx

@tf.function
def psi_p2_grad_y(x,y):
    psi_2_AM = psi_p2(x,y)
    psi_p2_dy = tf.gradients(psi_2_AM,y)[0]
    return psi_p2_dy
...
def psi_AM_y_1(x, y):
    ...
    return tf.sin(theta) / (4*r)

@tf.function
def psi_AM_y_1_grad_x(x,y):
    psi_AM_y1 = psi_AM_y_1(x,y)
    return tf.gradients(psi_AM_y1,x)[0]

@tf.function
def psi_AM_y_1_grad_y(x,y):
    psi_AM_y1 = psi_AM_y_1(x,y)
    return tf.gradients(psi_AM_y1,y)[0] 
...
def T_matrix(modes, x, y):
    ...
    AM_v_mode_x = phi_AM_vx_mode(x,y)
    AM_v_mode_y = phi_AM_vy_mode(x,y)
...
def M_matrix(mean,modes,x,y):
    ...
    for k in range(N_modes):
        q_vel_AM_1 = Q_velocity_mean_AM_1(mean, x, y)
        q_vel_AM_2 = Q_velocity_mean_AM_2(mean, x, y)
        mode_k = vel_mode_vector(modes[k],x,y)
        M_matrix[k, N_modes + 2 - 1] = M_matrix[k, N_modes + 2 - 1] + inner_product(q_vel_AM_1, mode_k )
        M_matrix[k, N_modes + 4 - 1] = M_matrix[k, N_modes + 4 - 1] + inner_product(q_vel_AM_2, mode_k ) 
...
def Q_matrix(modes, x, y):
    ...
    for k in range(N_modes):
        ...
        # i = N + 2, j in [1,N]
        for j in range(N_modes):
            q_AM_j_vector = phi_AM1_nabla_phi_i(modes, j, x, y)
            Q_matrix[k, N_modes+2-1, j] = Q_matrix[k, N_modes+2-1, j] + inner_product(q_AM_j_vector, mode_u_k)
        # j = N + 2 , i in [1,N]
        for i in range(N_modes):
            q_i_AM_vector = phi_i_nabla_phi_AM1(modes, i, x, y)
            Q_matrix[k, i, N_modes+2-1] = Q_matrix[k, i, N_modes+2-1] + inner_product(q_i_AM_vector, mode_u_k)
        
        # 2nd AM mode --------------------------------------------------------------------------------------------------
        # i = N + 4, j in [1,N]
        for j in range(N_modes):
            q_AM_j_vector = phi_AM2_nabla_phi_i(modes, j, x, y)
            Q_matrix[k, N_modes+4-1, j] = Q_matrix[k, N_modes+4-1, j] + inner_product(q_AM_j_vector, mode_u_k)
        # j = N + 4 , i in [1,N]
        for i in range(N_modes):
            q_i_AM_vector = phi_i_nabla_phi_AM2(modes, i, x, y)
            Q_matrix[k, i, N_modes+4-1] = Q_matrix[k, i, N_modes+4-1] + inner_product(q_i_AM_vector, mode_u_k)
        

        # i = j = N+2
        q_AM1_AM1_vector = phi_AM1_nabla_phi_AM1(x, y)
        # print(inner_product(q_AM_AM_vector, mode_u_k))
        Q_matrix[k, N_modes+2-1, N_modes+2-1] = Q_matrix[k, N_modes+2-1, N_modes+2-1] +\
             inner_product(q_AM1_AM1_vector, mode_u_k)
        # i = N + 2, j = N + 4
        q_AM1_AM2_vector = phi_AM1_nabla_phi_AM2(x,y)
        Q_matrix[k, N_modes+2-1, N_modes+4-1] = Q_matrix[k, N_modes+2-1, N_modes+4-1] +\
        inner_product(q_AM1_AM2_vector, mode_u_k)
        # i = N + 4, j = N + 2
        q_AM2_AM1_vector = phi_AM2_nabla_phi_AM1(x,y)
        Q_matrix[k, N_modes+4-1, N_modes+2-1] = Q_matrix[k, N_modes+4-1, N_modes+2-1] +\
        inner_product(q_AM2_AM1_vector, mode_u_k)
        # i = j = N + 4
        q_AM2_AM2_vector = phi_AM2_nabla_phi_AM2(x,y)
        Q_matrix[k, N_modes+4-1, N_modes+4-1] = Q_matrix[k, N_modes+4-1, N_modes+4-1] +\
        inner_product(q_AM2_AM2_vector, mode_u_k)
...
def Beta_matrix(modes, x, y):
    ...
    psi_AM1_p1_dx = psi_p1_grad_x(x,y)
    psi_AM1_p1_dy = psi_p1_grad_y(x,y)
    
    psi_AM2_p1_dx = psi_AM_y_1_grad_x(x,y)
    psi_AM2_p1_dy = psi_AM_y_1_grad_y(x,y)
    

    grad_psi_AM1_p1 = tf.cast(tf.concat([psi_AM1_p1_dx, psi_AM1_p1_dy],axis=0),dtype=tf.float64)
    grad_psi_AM2_p1 = tf.cast(tf.concat([psi_AM2_p1_dx, psi_AM2_p1_dy],axis=0),dtype=tf.float64)
    for k in range(N_modes):
        mode_k = tf.cast(vel_mode_vector(modes[k], x, y),tf.float64)
        beta_matrix[k,N_modes+2-1] = beta_matrix[k,N_modes+2-1] + inner_product(grad_psi_AM1_p1, mode_k)
        beta_matrix[k,N_modes+4-1] = beta_matrix[k,N_modes+4-1] + inner_product(grad_psi_AM2_p1, mode_k)

    return tf.constant(beta_matrix, tf.float64)

def Chi_matrix(modes, x, y):
    chi_matrix = np.zeros((N_modes, N_modes + 4, N_modes + 4))

    # The psi_2 modes are the same for the 2 Added Mass modes (in the x and y direction)
    # So it's the same coefficients in the chi matrix
    psi_p2_dx = psi_p2_grad_x(x,y)
    psi_p2_dy = psi_p2_grad_y(x,y)

    grad_psi_p2 = tf.cast(tf.concat([psi_p2_dx, psi_p2_dy],axis=0),tf.float64)
    for k in range(N_modes):
        mode_k = tf.cast(vel_mode_vector(modes[k], x, y),tf.float64)
        chi_matrix[k,N_modes+2-1,N_modes+2-1] = chi_matrix[k,N_modes+2-1,N_modes+2-1]+ inner_product(grad_psi_p2, mode_k)
        chi_matrix[k,N_modes+4-1,N_modes+4-1] = chi_matrix[k,N_modes+4-1,N_modes+4-1]+ inner_product(grad_psi_p2, mode_k)
...
def D_matrix(modes, x, y):
    ...
    for k in range(N_modes):
        ...
        d_phi_AM_x_dx = tf.concat([phi_AM_v1_x_grad_x(x, y), phi_AM_v1_y_grad_x(x, y)],axis=0)
        D_matrix[k, N_modes+1, N_modes+1] += inner_product(d_phi_AM_x_dx, mode_k)

        d_phi_AM_y_dx = tf.concat([phi_AM_v2_x_grad_x(x, y), phi_AM_v2_y_grad_x(x, y)],axis=0)
        D_matrix[k, N_modes+1, N_modes+3] += inner_product(d_phi_AM_y_dx, mode_k)

        d_phi_AM_x_dy = tf.concat([phi_AM_v1_x_grad_y(x, y), phi_AM_v1_y_grad_y(x, y)],axis=0)
        D_matrix[k, N_modes+3, N_modes+1] += inner_product(d_phi_AM_x_dy, mode_k)

        d_phi_AM_y_dy = tf.concat([phi_AM_v2_x_grad_y(x, y), phi_AM_v2_y_grad_y(x, y)],axis=0)
        D_matrix[k, N_modes+3, N_modes+3] += inner_product(d_phi_AM_y_dy, mode_k)
...
# def sigma_x(modes):
#     """
#     Computes a line that corresponds to the N + 2th  line in the Lambda matrix 
#     This line's coefficients are that of the equation of motion of the cylinder in x 
#     """
#     M = param.Mass_number
#     sigma_x_vect = np.zeros(N_modes+4)
    
#     for i in range(N_modes):
#         gamma_xi = gamma_POD_i_x(modes, i)
        
#         f_visc_i_x = viscosity_integration_x_fast(modes, i)/param.Re
        
#         sigma_x_vect[i] = - 2/(np.pi*(1 +M )) * gamma_xi + 2/(np.pi*(1 +M )) * f_visc_i_x
...
# def sigma_y(modes):
#     """
#     Computes a line that corresponds to the N + 4th  line in the Lambda matrix 
#     This line's coefficients are that of the equation of motion of the cylinder in x 
#     """
#     M = param.Mass_number
...
#     for i in range(N_modes):
#         gamma_yi = gamma_POD_i_y(modes, i)

#         f_visc_i_y = viscosity_integration_y_fast(modes, i)/param.Re
        
#         sigma_y_vect[i] = - 2/(np.pi*(1 +M )) * gamma_yi + 2/(np.pi*(1 +M )) * f_visc_i_y
...
#     lambda_matrix[N_modes +1,:] = sigma_x(modes) 
    
#     lambda_matrix[N_modes+2,N_modes+3] = 1.0
#     lambda_matrix[N_modes +3,:] = sigma_y(modes) 
    
#     return lambda_matrix


# def Lambda_matrix_update(mean_mode, modes, x, y):
#     inv_Re = 1/param.Re
#     lambda_matrix = np.zeros((N_modes+4,N_modes+4))
